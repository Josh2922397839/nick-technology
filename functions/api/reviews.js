...
export async function onRequestGet(context) {
  try {
    ...
    // Query all reviews from the D1 database
    // Sorted by date descending, limit to 100 for safety and performance
    const { results } = await db.prepare("SELECT * FROM reviews ORDER BY date DESC, id DESC LIMIT 100").all();

    return new Response(JSON.stringify(results), {
      headers: { 
        "Content-Type": "application/json",
        "Cache-Control": "public, max-age=5" // Cache briefly for performance
      }
    });
  } catch (error) {
...
export async function onRequestPost(context) {
  try {
    ...
    if (secretKey) {
      ...
      // Verify the token by POSTing to Cloudflare's Turnstile siteverify API
      ...
      console.warn("TURNSTILE_SECRET_KEY environment variable is not defined. Skipping verification in development.");
    }
    ...
    // 4. Insert the new review into the D1 Database
    await db.prepare(
